...
from adafruit_servokit import ServoKit # import ServoKit from adafruit_servokit
# ##################################################################################
...
def load_servos_parameters(print_out):
    ...
    frame_width=0.020,                           # min pulse width variation accepted
    ...
    def servo_start_pos(start_pos):
        ...
        global t_servo, b_servo, t_servo_open, b_home

        t_servo.value = t_servo_open  # top servo is positioned to the open position
        b_servo.value = b_home        # bottom servo is positioned to the home position
        time.sleep(1)                 # time to let the servos reaching the start position
        return


    def cam_led_test():
       
        global top_cover_led

        top_cover_led.value = 1  # led is turned on
        time.sleep(0.1)          # short delay
        top_cover_led.value = 0  # led is turned off
        return
    ...
    # Main function to control the servos based on the movements string
    def control_servos(movements):
        
        global t_servo, b_servo, t_servo_close, t_servo_open, t_servo_flip, t_servo_read, t_servo_rel
        global b_servo_CCW, b_servo_CW, b_home, b_rel_CW, b_rel_CCW, b_extra_home_CW, b_extra_home_CCW
        global b_servo_home, b_servo_stopped, b_servo_CW_pos, b_servo_CCW_pos, b_spin_time, b_rotate_time, b_rel_time
...
def load_servos_parameters(print_out):
    ...
    except:   # exception will be raised if json keys differs, or
        """ Parameters are imported from a json file, to make easier to list/document/change the variables
        that are expected to vary on each robot.
        These servo_settings are under a function, instead of root, to don't be executed when this script is used from CLI
        with arguments; In this other case the aim is to help setting the servo to their mid position."""
...
except:   # exception will be raised if json keys differs, or                 # bottom servo timers
